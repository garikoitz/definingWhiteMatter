...
SUBJDIR   = '/Users/glerma/soft/definingWhiteMatter/local'
...
for s in subs:
    ...
    aparc    = join(s,'T1w','aparc+aseg.nii.gz ')
    binaparc = join(s,'T1w', 'segmentation.nii.gz')
    ...
    binaparcdil = join(s,'T1w', 'segmentation_dilated.nii.gz')
    ...
    tractogram = {'det':join(s,'tracking-deterministic-'+fmm+'mm.tck'),
                  'prob':join(s,'tracking-probabilistic-'+fmm+'mm.tck')}
    # Obtaih tracts dict and loop them
    tracts = R2T.keys()
    for tr in tracts:        
        # Read required ROIs
        ROI1   = join(s,'ROIs', R2T[tr][0])
        ROI2   = join(s,'ROIs', R2T[tr][1])
        ...
        for dodp in doDetProb:
            ...
            if useCortex:
                tracks_out = join(s,'tracts-'+fmm,dodp,tr+'_cortex.tck')
            else:
                tracks_out = join(s,'tracts-'+fmm,dodp,tr+'.tck')
